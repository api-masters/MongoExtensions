...
use MongoExtensions\Mapping\Driver\File as FileDriver;
use MongoExtensions\Mapping\Driver\AnnotationDriverInterface;
...
class ExtensionMetadataFactory
{
    /**
     * Extension driver
     * @var \MongoExtensions\Mapping\Driver
     */
    protected $driver;
     ...
     * @throws \MongoExtensions\Exception\RuntimeException if driver was not found in extension
     * @return \MongoExtensions\Mapping\Driver
     */
    protected function getDriver($omDriver)
    {
        ...
        if ($omDriver instanceof MappingDriverChain || $driverName == 'DriverChain') {
            ...
            if (!class_exists($driverClassName)) {
                ...
                if (!class_exists($driverClassName)) {
                    throw new \MongoExtensions\Exception\RuntimeException("Failed to fallback to annotation driver: ({$driverClassName}), extension driver was not found.");
                }
            }
