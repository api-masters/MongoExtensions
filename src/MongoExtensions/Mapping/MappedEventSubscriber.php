...
use MongoExtensions\Mapping\Event\AdapterInterface;
...
abstract class MappedEventSubscriber implements EventSubscriber
{
     ...
     *
     * @throws \MongoExtensions\Exception\InvalidArgumentException - if event is not recognized
     *
     * @return \MongoExtensions\Mapping\Event\AdapterInterface
     */
        ...
        if (preg_match('@Doctrine\\\([^\\\]+)@', $class, $m) && in_array($m[1], array('ODM', 'ORM'))) {
            if (!isset($this->adapters[$m[1]])) {
                ...
                if (!class_exists($adapterClass)) {
                    $adapterClass = 'MongoExtensions\\Mapping\\Event\\Adapter\\'.$m[1];
                }
            ...
            throw new \MongoExtensions\Exception\InvalidArgumentException('Event mapper does not support event arg class: '.$class);
        }
    }
        ...
        if (null === self::$defaultAnnotationReader) {
            if (version_compare(\Doctrine\Common\Version::VERSION, '2.2.0-DEV', '>=')) {
                ...
                \Doctrine\Common\Annotations\AnnotationRegistry::registerAutoloadNamespace(
                    'MongoExtensions\\Mapping\\Annotation',
                    __DIR__.'/../../'
                );
                ...
                $reader->setAnnotationNamespaceAlias('MongoExtensions\\Mapping\\Annotation\\', 'gedmo');
